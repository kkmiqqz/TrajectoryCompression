...
// ==================== OPW-TR Algorithm ====================
vector<int> OPW_TR(const vector<GPSPoint>& points, double epsilon) {
    int originalIndex = 0;
    vector<int> simplified;
    simplified.push_back(originalIndex);

    int e = originalIndex + 2;
    while (e < (int)points.size()) {
        int i = originalIndex + 1;
        bool condOPW = true;

        while (i < e && condOPW) {
            if (calcSED(points[originalIndex], points[i], points[e]) > epsilon) {
                condOPW = false;
            }
            else {
                i++;
    ...
    simplified.push_back((int)points.size() - 1);
    return simplified;
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        //filename = "E:\\大四\\毕设\\提交\\TestOthers\\Geolife_100k_with_timestamp.csv";
        //filename = "E:\\大四\\毕设\\提交\\TestOthers\\Trajtory_100k_with_id.csv";
        filename = "E:\\大四\\毕设\\提交\\TestOthers\\WX_taxi_100k_with_id.csv";
        epsilonMeters = 1.0;
        saveFilename = "outputTR.txt";  // Use relative path
        maxPoints = -1;
    }
    else {
        filename = argv[1];
    ...
    cout << "\n========================================" << endl;
    cout << "OPW-TR Trajectory Compression" << endl;
    cout << "========================================" << endl;
    ...
    cout << "Epsilon threshold: " << epsilonMeters << " meters" << endl;

    // 4. Execute OPW-TR algorithm for each segment with timing
    ...
    for (size_t segIdx = 0; segIdx < trajectorySegments.size(); segIdx++) {
        ...
        vector<int> segmentKeptIndices = OPW_TR(segment, epsilonDegrees);
    ...
    if (outFile.is_open()) {
        ...
        outFile << "\n========================================\n";
        outFile << "OPW-TR Performance Statistics\n";
        outFile << "========================================\n";
    ...
    // 6. Console output
    cout << "\n========================================" << endl;
    cout << "OPW-TR Compression Results" << endl;
    cout << "========================================" << endl;
    cout << setprecision(6);
    cout << "\n--- Data ---" << endl;
